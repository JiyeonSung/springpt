...
<script>
	$(document).ready(function(){
		...
		CKEDITOR.replace("product_detail", ckeditor_config);
						...
						<div class="box box-primary">
							...
							<form id='editForm' role="form" action="/admin/product/edit" method="post" enctype="multipart/form-data">
								<div class="box-body">
									...
									<div class="form-group">
										<input name="product_num" type="hidden" value="${vo.product_num}" />
										...
										<select class="form-control" id="mainCategory" name="cat_parcode" style="width:30%; margin-right:10px; display: inline-block;" >
											<option value="default">1차 카테고리 선택</option>
											<c:forEach items="${cateList}" var="list">
												<option value="${list.cat_code}"<c:out value="${vo.cat_parcode == list.cat_code?'selected':''}"/>>${list.cat_name}</option>
											</c:forEach>
										</select>
										<select class="form-control" id="subCategory" name="cat_code" style="width: 30%; display: inline-block;">
										 	...
										 	<c:forEach items="${subCateList}" var="subList">
										  		<option value="${subList.cat_code}"<c:out value="${vo.cat_code == subList.cat_code?'selected':''}"/>>${subList.cat_name}</option>
										 	</c:forEach>
										</select>
									</div>
									<div class="form-group">
										<label for="exampleInputEmail1">Product Name</label> <input
											type="text" id="product_name" name="product_name" class="form-control"
											value="${vo.product_name}">
									</div>
									<div class="form-group">
										<label for="exampleInputEmail1">Company</label> <input
											type="text" id="product_company" name="product_company" class="form-control"
											value="${vo.product_company}">
									</div>
									<div class="form-group">
										...
										<input style="width:40%; margin-right:10px; display: inline-block;"
											type="text" id="product_price" name="product_price" class="form-control" 
											value="${vo.product_price}" />
										<input style="width:40%; display: inline-block;"
											type="text" id="product_discount" name="product_discount" class="form-control "
											value="${vo.product_discount}" />
									</div>
									<div class="form-group">
										<label for="exampleInputPassword1">Detail</label>
										<textarea class="form-control" id="product_detail" name="product_detail" rows="8">
											${vo.product_detail}</textarea>
									</div>
									<div class="form-group">
										<input type="hidden" name="product_img" value="${vo.product_img}" />	
									...
									<div class="form-group">
										...
										<input style="width:30%; margin-right:10px; display: inline-block;"
											type="text" id="product_amount" name='product_amount' class="form-control" 
											value="${vo.product_amount}" />
										<!-- 상품 판매 여부 -->
										<select class="form-control" id="product_buy" name="product_buy" style="width: 15%; display: inline-block;">
										  <option 
										  	<c:out value="${vo.product_buy == 'Y'?'selected':''}"/>>Y</option>
										  <option
										  	<c:out value="${vo.product_buy == 'N'?'selected':''}"/>>N</option>
										</select>
									</div>
									<div class="form-group">
										...
										<span class="form-control" style="width:40%; margin-right:10px; display:inline-block;">
											<fmt:formatDate value="${vo.product_date}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
										<span class="form-control" style="width:40%; display: inline-block;">
											<fmt:formatDate value="${vo.product_update}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
										<!-- 
										<input type="text" id="product_date" name="product_date" class="form-control" 
											value="${vo.product_date}" readonly />
											 -->
									</div>
								</div>
