...
<script>
	$(document).ready(function(){
		...
		$("#btn_edit_check").on("click", function(){
			...
			var product_buy = $("#product_buy:selected").val();
			
			// 체크 된 상품의 value(product_num)을 가져옴
			$("input[name='check']:checked").each(function(i){
				var product_num = $(this).val();
				var product_amount = $("input[name='amount_"+product_num+"']").val();
				var product_buy = $("select[name='buy_"+product_num+"']").val();
				
				checkArr.push(product_num);			
				amountArr.push(product_amount);			
				buyArr.push(product_buy);
		...
		$("#btn_delete_check").on("click", function(){
			...
			if(result){
				...
				$("input[name='check']:checked").each(function(i){
					var product_num = $(this).val();
					var product_img = $("input[name='img_"+product_num+"']").val();
					
					checkArr.push(product_num);
					imgArr.push(product_img);
				});
...
<script>
	/* 상품 수정 버튼 클릭 시 */
	var clickEdit = function(product_num){
		var url = '/admin/product/edit${pm.makeSearch(pm.cri.page)}&product_num=' + product_num;
		//alert(url);
		location.href= url;
	};
</script>
							...
							<div class="box-body">
								<table class="table table-striped text-center">
									...
									<c:forEach items="${productList}" var="productVO">
										<tr>
											<td><input type="checkbox" name="check" class="check" value="${productVO.product_num}" style=""></td>
											<td class="col-md-1">${productVO.product_num}</td>
											<td class="col-md-2">
												<img src="/admin/product/displayFile?fileName=${productVO.product_img}" style="width:80px;">
												<input type="hidden" name="img_${productVO.product_num}" value="${productVO.product_img}" />
											</td>
											<td class="col-md-2"><a
												href="/admin/product/read${pm.makeSearch(pm.cri.page)}&product_num=${productVO.product_num}"
												style="color: black;"> ${productVO.product_name} </a></td>
											<td class="col-md-1">${productVO.product_price}</td>
											<td class="col-md-1">${productVO.product_discount}</td>
											<td class="col-md-2">${productVO.product_company}</td>
											<td><input name="amount_${productVO.product_num}" type="number" style="width:80px; height:34px; padding-left:5px;" value="${productVO.product_amount}" /></td>
											<td>
												<select class="form-control" name="buy_${productVO.product_num}" style="width: 60px; display: inline-block;">
												  <option <c:out value="${productVO.product_buy == 'Y'?'selected':''}"/>>Y</option>
												  <option <c:out value="${productVO.product_buy == 'N'?'selected':''}"/>>N</option>
												</select>
											</td>
											<td class="col-md-2">
												<form class="deleteForm" method="post"
													...
													<input type="hidden" name="product_img"
														value="${productVO.product_img}">

													<button type="button" name="btn_edit" class="btn btn-default" onclick="clickEdit(${productVO.product_num});">수정</button>
