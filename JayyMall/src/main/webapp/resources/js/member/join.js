$(document).ready(function() {
	...
	$("#btn_checkId").on("click", function(){
		...
		// ajax 방식은 제어의 흐름이 클라이언트 시작 -> 서버 요청 -> 클라이언트  종료
		// ajax 방식은 서버요청에 실행되는 코드가 response.redirect, redirect: 주소 이동 구문을 사용 안하거나,
		// 동작 되어서는 안된다.
		$.ajax({
			url: '/member/checkIdDuplicate',
			type: 'post',
			dataType: 'text',  // '/member/checkIdDuplicate' 주소에서 넘어오는 리턴값의 형식
			data: {mem_id : mem_id},  // javascript object 표현 구문 (www.w3chool.com 참고)
				   // 키에 값을 넘겨줌.
			success : function(data){
				if(data== 'SUCCESS'){
					...
					// 아이디를 중복체크 하고 나서 아이디 텍스트박스 읽기모드, 중복체크 비활성화
					$("#mem_id").attr("readonly", true);
					$("#btn_checkId").attr("disabled", true);

					isCheckId = "true";  // 아이디 중복체크를 하는 용도
				} else {
					// 사용 불가능 - 이미 존재하는 아이디
	...
	$("#btn_sendAuthCode").on("click", function(){
		...
		if($("#mem_email").val()=="" || $("#mem_email").val()== null){
			// 메세지를 alert()를 사용하지 않고, 임의의 위치에 출력하는 형태
		...
		$.ajax({
			...
			type: 'post',
			dataType: 'text',
			data: {receiveMail : receiveMail},  // {key : value(변수)}  EmailDTO와 이름이 동일해야 함.
			success: function(data){
				$("#email_authcode").show();  // login.jsp에 존재
				$("#authcode_status").css("color", "grey");
	...
	$("#btn_submit").on("click", function(){
		...
		var mem_nickname = $("#mem_nickname");
		var mem_email = $("#mem_email");
		var mem_authcode = $("#mem_authcode");
		var mem_tel = $("#mem_tel");
		var mem_zipcode = $("input[name='mem_zipcode']");
		var mem_basicadd = $("input[name='mem_basicadd']");
		var mem_detailadd = $("input[name='mem_detailadd']");
		...
		} else if(isCheckId =="false"){  // 아이디 중복기능 사용 여부 확인
		...
		} else if(mem_nickname.val()==null || mem_nickname.val()==""){
			alert("닉네임을 입력해주세요.");
			mem_nick.focus();
		...
		} else if(isCheckEmail=="false"){  // 이메일 인증 확인 여부
			alert("이메일인증을 해주세요.");
		...
		} else if(mem_basicadd.val()==null || mem_basicadd.val()==""){
			alert("주소를 입력해주세요.");
			$("#btn_postCode").focus();
			
		} else if(mem_detailadd.val()==null || mem_detailadd.val()==""){
			alert("상세 주소를 입력해주세요.");
			mem_addr_d.focus();
