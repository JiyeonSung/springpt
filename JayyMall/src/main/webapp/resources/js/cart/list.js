...
var updatePrice = function(){
	...
	$("input[name='check']:checked").each(function(i){
		...
		totalPD += parseInt($("input[name='pd_"+cart_code+"']").val())*amount;
	});

	$("#totalPrice").html(numberFormat(totalPrice) + "원");
	$("#totalDiscount").html(numberFormat(totalDiscount) + "원");
	$("#totalPD").html(numberFormat(totalPrice-totalDiscount) + "원");
};
...
/* 상품-구매 버튼 클릭 */
var clickBuyBtn = function(product_num, cart_code){

	var cart_amount = $("input[name='cart_amount_"+cart_code+"']").val();
	var url = "/order/buyFromCart?product_num="+product_num+"&ord_amount="+cart_amount;
	location.href = url;
}