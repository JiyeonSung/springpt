...
@RestController
@RequestMapping("/admin/order/chart/*")
public class GoogleChartController {
	...
	@RequestMapping("product_price_list")
    public JSONObject product_price_list() {
	...
	*/
	...
	@RequestMapping(value="/product_price_list_2" ,method=RequestMethod.GET)
	public ResponseEntity<JSONObject> money_list(){
		...
		List<GoogleChartDTO> items = new ArrayList<GoogleChartDTO>();
        
        Random rand = new Random();
        
        for(int i=0;i < 5; i++) {
        	GoogleChartDTO cart = new GoogleChartDTO();
...
*/ 	
		...
		*/
		...
		for(GoogleChartDTO  dto : items){
			// {"v":"Mushrooms"}
			JSONObject name =new JSONObject();
			name.put("v", dto.getProduct_name()); //상품이름 -> v 객체 
			// {"v":3}
			JSONObject price =new JSONObject(); 
			price.put("v", dto.getProduct_price()); //가격 ->v 객체
	...
	// 클라이언트로 보낼 JSON 데이터 형식의 작업구문
		@SuppressWarnings("unchecked")
		@RequestMapping(value="/product_price_array" ,method=RequestMethod.GET)
	...
	*/ 	
	...
	public ResponseEntity<String[][]> getArray2(){
		...
		for(GoogleChartDTO  dto : items){
			...
			cell.put(dto.getProduct_name(),dto.getProduct_price()); //상품이름 -> v 객체 
